...
public class NoteController {
	...
	public ResponseEntity<Note> addNote(@RequestBody Note note) {
		try {
			...
			URI uri = linkTo(NoteController.class).slash(savedNote.getId()).toUri();
    ...
    @ApiResponses({ 
            @ApiResponse(responseCode = "204", description = "Note was updated", content = @Content),
            @ApiResponse(responseCode = "404", description = "Note not found", content = @Content),
            @ApiResponse(responseCode = "500", description = "Something went wrong", content = @Content)})
	@PutMapping("/{id}")
	public ResponseEntity<Void> getNote(@PathVariable long id, @RequestBody Note note) {
